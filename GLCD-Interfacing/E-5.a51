...
org 002bh
	...
	main:
		...
		here:push psw
			...
			lcall calc_pos
			lcall display_dot
			lcall clr_display_dot ;this is the new subroutine written to c
			pop psw
...
;		ret ;for check busy flag
		...
		clr_display_dot:
			...
			mov a,13h ;reg-3 of reg-bank-2
			anl a,#0fh
			acall set_column
			
			mov a,14h ;reg-4 pf reg-bank-2
			acall set_pg_cntrl
			
			mov r0,#8d ;individual column counter
			clr a
			clr_dot_loop:acall datawrt
			djnz r0,clr_dot_loop
			
			pop psw
		
		ret ;for clr_display_dot
...
;		*****For Our Case p0.0->Right, p0.1->Left, p0.2->Up, p0.3->Down*****
				...
				mov a,r1 ;store the dots previous coordinates at these registers
				mov r3,a
