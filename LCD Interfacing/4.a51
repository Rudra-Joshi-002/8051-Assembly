...
org 002bh ;leaving space for all isr's
	...
	org 0003h ;isr for external interrupt-0 starts here
		
		ljmp isr_ext_0
		return: reti ;return to next instruction before interrupt was called that was supposed to be executed
		...
		mov dptr,#keypad ;initialize dptr with loo-up table address
		mov r0,#00h
		mov r1,#08h
		
		no_release: mov a,p0 ;checks if any key is currently pressed
		cjne a,#0ffh,no_release
		acall dboun
		
		wait: mov a,p0 ;waits until is key is pressed
		cjne a,#0ffh,indentify
		sjmp wait
		
		indentify: acall dboun ;now the program serves to check 
		mov a,p2 ;which key is pressed
		...
		dboun: mov r6,#10d ;delay subroutine
		dloop2:mov r7,#250d
		dloop1:nop
		nop
		djnz r7,dloop1
		...
		exit: clr psw.3; reset the register bank for lcd display puropses
		clr psw.4
		ljmp return
